...
"public class URLLoader extends flash.events.EventDispatcher",2,function($$private){var is=joo.is,$$bound=joo.boundMethod,trace=joo.trace;return[function(){joo.classLoader.init(flash.events.IOErrorEvent,flash.events.Event,js.XMLHttpRequest);}, 
   ...
   *
   */
  "public function URLLoader",function URLLoader$(request/*:URLRequest = null*/) {flash.events.EventDispatcher.call(this);if(arguments.length<1){request = null;}
    if (request) {
      this.load(request);
    }
  },
  ...
  "private function readyStateChanged",function readyStateChanged()/*:void*/ {
    ...
    var eventType/*:String*/ = this.createEvent$2();
    if (eventType) {
      this.dispatchEvent(new flash.events.Event(eventType, false, false));
    }
  },

  "private function createEvent",function createEvent()/*:String*/ {
